...
pbp_clean['interception'] = np.where((pbp_clean['interception'] == 1) & (pbp_clean['drive_end_transition']=='INTERCEPTION'),1,0)
...
pre_df = pbp_clean[['game_id','play_id','season','name','posteam','down','yardline_100','game_seconds_remaining',
                    ...
                    'time_to_throw','pass','interception']]
pre_df.isna().sum()
...
qbs = int_preds[(int_preds['season']==2023)].groupby(['name','posteam']).agg({'pass':'count','interception':'sum','int_pred':'sum','int_oe':'sum'}).reset_index().sort_values('int_oe', ascending= True)
qbs = qbs[(qbs['pass']>=250)]
...
for i, txt in enumerate(types):
    ax.annotate(txt, (x[i], y[i]), xytext=(15,15), textcoords='offset points')
    
# Plot parameters
plt.xlim(4, 18);
plt.ylim(-6,6);
plt.title("2023 NFL Season - INTs vs. INTs Over Expected", fontdict={'fontsize':35});
plt.xlabel("INTs", fontdict={'fontsize':21});
plt.ylabel("INTs Over Expected", fontdict={'fontsize':21});

#Reference /Best Fit Line
a, b = np.polyfit(x, y, 1)
plt.plot(x, a*x+b) 


# -Median for predicted INT is higher than the play is predicted to not have an INT. Good!
# 
# -All QBs below the reference line, out performed their predict INT performance.
# 
# -Further improvements of this model could be to add features such as pressured throws and Ccoverages faced. Also attempt to heat map other potential significant variables in EDA
